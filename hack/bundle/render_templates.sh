...
if [[ -z ${SPIRE_SERVER_IMAGE} ]] || [[ -z ${SPIRE_AGENT_IMAGE} ]] || [[ -z ${SPIFFE_CSI_DRIVER_IMAGE} ]] || [[ -z ${SPIRE_OIDC_DISCOVERY_PROVIDER_IMAGE} ]] || [[ -z ${SPIRE_CONTROLLER_MANAGER_IMAGE} ]] || [[ -z ${SPIFFE_HELPER_IMAGE} ]] || [[ -z ${NODE_DRIVER_REGISTRAR_IMAGE} ]] || [[ -z ${ZERO_TRUST_WORKLOAD_IDENTITY_MANAGER_IMAGE} ]]; then
	log_error "\"${SPIRE_SERVER_IMAGE}\" or \"${SPIRE_AGENT_IMAGE}\"  or \"${SPIFFE_CSI_DRIVER_IMAGE}\"  or \"${SPIRE_OIDC_DISCOVERY_PROVIDER_IMAGE}\"  or \"${SPIRE_CONTROLLER_MANAGER_IMAGE}\"  or \"${SPIFFE_HELPER_IMAGE}\" or \"${NODE_DRIVER_REGISTRAR_IMAGE}\" or \"${ZERO_TRUST_WORKLOAD_IDENTITY_MANAGER_IMAGE}\" is not set"
	exit 1
fi
